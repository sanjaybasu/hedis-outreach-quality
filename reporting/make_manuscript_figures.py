"""Regenerate the manuscript-final Figures 2 and 3 (five reported measures; no PHI).

Figure 2 (file figure1_reaim_reach_response.png): reach and member response by measure.
Figure 3 (file figure2_burden_absorbed.png): manual per-patient identification time absorbed.
...
Figure 1 (figure1_did_eventstudy.png, the event study) is unaffected by the five-measure
...
def figure2():
    ...
    fig.savefig(os.path.join(OUT, "figure1_reaim_reach_response.png"), bbox_inches="tight")
    plt.close(fig)
...
def figure3():
    ...
    ax.set_title(f"Manual per-patient identification time absorbed\n"
                 ...
                 fontsize=19, pad=14)
    _despine(ax); fig.tight_layout()
    fig.savefig(os.path.join(OUT, "figure2_burden_absorbed.png"), bbox_inches="tight")
    plt.close(fig)
...
if __name__ == "__main__":
    figure2()
    figure3()
    print("Wrote figure1_reaim_reach_response.png and figure2_burden_absorbed.png to", OUT)
